...
#include "parser_typedef.h"
        ...
        define api.prefix{ff_} % define parse.error verbose %
        define api.pure full % define api.token.prefix{SQL_} % locations
...
% lex - param{yyscan_t scanner} % parse - param{Program * result} % parse -
    param{yyscan_t scanner} % union FF_STYPE {
  long ival;
  char* sval;
  double fval;
  Program* program_t;
  ...
  OptEnforced* opt_enforced_t;
    ...
    token PRECEDING EXCEPT NUMERIC OP_LESSTHAN ACTION BEFORE OP_GREATEREQ
        CHECK %
    ...
    token NOTHING HAVING % token<ival> INTLITERAL
    ...
    % type<program_t> program % type<stmtlist_t> stmtlist % type<stmt_t> stmt %
    type<create_stmt_t> create_stmt % type<drop_stmt_t> drop_stmt %
    type<alter_stmt_t> alter_stmt % type<select_stmt_t> select_stmt %
    ...
    type<window_def_list_t> window_def_list % type<window_def_t> window_def %
    type<window_name_t> window_name % type<window_t> window %
    ...
    type<frame_bound_end_t> frame_bound_end % type<frame_bound_t> frame_bound %
    ...
    type<where_clause_t> where_clause % type<from_clause_t> from_clause %
    type<table_ref_t> table_ref % type<opt_index_t> opt_index %
    type<opt_on_t> opt_on % type<opt_using_t> opt_using %
    type<column_name_list_t> column_name_list %
    type<opt_table_prefix_t> opt_table_prefix % type<join_op_t> join_op %
    type<opt_join_type_t> opt_join_type % type<expr_list_t> expr_list %
    ...
    type<order_item_list_t> order_item_list % type<order_item_t> order_item %
    type<opt_order_behavior_t> opt_order_behavior %
    type<opt_with_clause_t> opt_with_clause %
    type<cte_table_list_t> cte_table_list % type<cte_table_t> cte_table %
    ...
    type<table_option_t> table_option % type<opt_op_comma_t> opt_op_comma %
    ...
    type<lock_option_t> lock_option % type<opt_op_equal_t> opt_op_equal %
    type<trigger_events_t> trigger_events % type<trigger_name_t> trigger_name %
    ...
    type<drop_view_stmt_t> drop_view_stmt % type<insert_stmt_t> insert_stmt %
    type<insert_rest_t> insert_rest %
    type<super_values_list_t> super_values_list %
    type<values_list_t> values_list % type<opt_on_conflict_t> opt_on_conflict %
    type<opt_conflict_expr_t> opt_conflict_expr %
    type<indexed_column_list_t> indexed_column_list %
    type<indexed_column_t> indexed_column % type<update_stmt_t> update_stmt %
    type<alter_action_t> alter_action %
    type<alter_constant_action_t> alter_constant_action %
    type<column_def_list_t> column_def_list % type<column_def_t> column_def %
    ...
    type<opt_check_t> opt_check % type<constraint_type_t> constraint_type %
    ...
    type<constraint_name_t> constraint_name % type<opt_temp_t> opt_temp %
    type<opt_check_option_t> opt_check_option %
    type<opt_column_name_list_p_t> opt_column_name_list_p %
    type<set_clause_list_t> set_clause_list % type<set_clause_t> set_clause %
    type<opt_as_alias_t> opt_as_alias % type<expr_t> expr %
    type<operand_t> operand % type<cast_expr_t> cast_expr %
    type<scalar_expr_t> scalar_expr % type<unary_expr_t> unary_expr %
    type<binary_expr_t> binary_expr % type<logic_expr_t> logic_expr %
    type<in_expr_t> in_expr % type<case_expr_t> case_expr %
    type<between_expr_t> between_expr % type<exists_expr_t> exists_expr %
    type<function_expr_t> function_expr % type<opt_distinct_t> opt_distinct %
    type<opt_filter_clause_t> opt_filter_clause %
    type<opt_over_clause_t> opt_over_clause % type<case_list_t> case_list %
    type<case_clause_t> case_clause % type<comp_expr_t> comp_expr %
    ...
    type<int_literal_t> int_literal % type<float_literal_t> float_literal %
    type<opt_column_t> opt_column % type<trigger_body_t> trigger_body %
    type<opt_if_not_exist_t> opt_if_not_exist %
    type<opt_if_exist_t> opt_if_exist % type<identifier_t> identifier %
    type<as_alias_t> as_alias % type<table_name_t> table_name %
    type<column_name_t> column_name %
    type<opt_index_keyword_t> opt_index_keyword % type<view_name_t> view_name %
    type<function_name_t> function_name % type<binary_op_t> binary_op %
    type<opt_not_t> opt_not % type<name_t> name % type<type_name_t> type_name %
    ...
    % left OR % left AND % left NOT % nonassoc OP_NOTEQUAL MATCH LIKE OP_EQUAL %
    nonassoc OP_GREATERTHAN OP_LESSTHAN OP_GREATEREQ OP_LESSEQ %
    nonassoc ISNULL % nonassoc IS % left OP_ADD OP_SUB %
    left OP_MUL OP_MOD OP_DIVIDE % left OP_XOR % right UMINUS %
    left OP_LBRACKET OP_RBRACKET % left OP_RP OP_LP % nonassoc JOIN %
    nonassoc ON
...
stmtlist : stmt OP_SEMI stmtlist {
  $$ = new Stmtlist();
  $$->case_idx_ = CASE0;
  $$->stmt_ = $1;
  $$->stmtlist_ = $3;
...
select_stmt : select_no_parens % prec UMINUS {
  $$ = new SelectStmt();
  $$->case_idx_ = CASE0;
  $$->select_no_parens_ = $1;
}
...
select_with_parens : OP_LP select_no_parens OP_RP {
...
select_clause : SELECT opt_all_or_distinct select_target opt_from_clause
                    opt_where_clause opt_group_clause opt_window_clause {
  $$ = new SelectClause();
  $$->case_idx_ = CASE0;
...
select_target : expr_list {
  $$ = new SelectTarget();
  $$->case_idx_ = CASE0;
  $$->expr_list_ = $1;
};

opt_window_clause : window_clause {
  $$ = new OptWindowClause();
  $$->case_idx_ = CASE0;
  $$->window_clause_ = $1;
}
...
window_clause : WINDOW window_def_list {
  $$ = new WindowClause();
  $$->case_idx_ = CASE0;
  $$->window_def_list_ = $2;
};
...
window_def : window_name AS OP_LP window OP_RP {
  $$ = new WindowDef();
  $$->case_idx_ = CASE0;
  $$->window_name_ = $1;
  $$->window_ = $4;
...
window : opt_exist_window_name opt_partition opt_order_clause opt_frame_clause {
  $$ = new Window();
  $$->case_idx_ = CASE0;
...
opt_partition : PARTITION BY expr_list {
  $$ = new OptPartition();
  $$->case_idx_ = CASE0;
  $$->expr_list_ = $3;
}
...
opt_frame_clause : range_or_rows frame_bound_start {
  $$ = new OptFrameClause();
  $$->case_idx_ = CASE0;
  $$->range_or_rows_ = $1;
...
opt_exist_window_name : identifier {
...
opt_group_clause : GROUP BY expr_list opt_having_clause {
  $$ = new OptGroupClause();
  $$->case_idx_ = CASE0;
  $$->expr_list_ = $3;
...
| opt_table_prefix function_name OP_LP expr_list OP_RP opt_as_alias opt_on
        opt_using {
  $$ = new TableRef();
  $$->case_idx_ = CASE1;
  $$->opt_table_prefix_ = $1;
...
opt_using : USING OP_LP column_name_list OP_RP {
  $$ = new OptUsing();
  $$->case_idx_ = CASE0;
  $$->column_name_list_ = $3;
}
...
opt_table_prefix : table_ref join_op {
  $$ = new OptTablePrefix();
  $$->case_idx_ = CASE0;
  $$->table_ref_ = $1;
  $$->join_op_ = $2;
...
expr_list : expr opt_as_alias OP_COMMA expr_list {
  $$ = new ExprList();
  $$->case_idx_ = CASE0;
  $$->expr_ = $1;
  $$->opt_as_alias_ = $2;
...
opt_limit_row_count : LIMIT expr {
...
opt_order_clause : ORDER BY order_item_list {
  $$ = new OptOrderClause();
  $$->case_idx_ = CASE0;
  $$->order_item_list_ = $3;
}
...
order_item : expr opt_order_behavior opt_order_nulls {
  $$ = new OrderItem();
  $$->case_idx_ = CASE0;
  $$->expr_ = $1;
...
opt_with_clause : WITH cte_table_list {
  $$ = new OptWithClause();
  $$->case_idx_ = CASE0;
  $$->cte_table_list_ = $2;
}
...
cte_table : cte_table_name AS OP_LP select_stmt OP_RP {
  $$ = new CteTable();
  $$->case_idx_ = CASE0;
  $$->cte_table_name_ = $1;
  $$->select_stmt_ = $4;
...
cte_table_name : table_name opt_column_name_list_p {
  $$ = new CteTableName();
  $$->case_idx_ = CASE0;
  $$->table_name_ = $1;
...
create_table_stmt
    : CREATE opt_temp TABLE opt_if_not_exist table_name opt_table_option_list
          opt_ignore_or_replace AS select_stmt {
  $$ = new CreateTableStmt();
  $$->case_idx_ = CASE0;
  $$->opt_temp_ = $2;
  $$->opt_if_not_exist_ = $4;
...
| CREATE opt_temp TABLE opt_if_not_exist table_name OP_LP column_def_list
        opt_table_constraint_list OP_RP opt_table_option_list {
  $$ = new CreateTableStmt();
  $$->case_idx_ = CASE1;
  $$->opt_temp_ = $2;
  $$->opt_if_not_exist_ = $4;
...
create_index_stmt
    : CREATE opt_index_keyword INDEX table_name ON table_name OP_LP
          indexed_column_list OP_RP opt_index_option opt_extra_option {
  $$ = new CreateIndexStmt();
  $$->case_idx_ = CASE0;
...
create_trigger_stmt
    : CREATE TRIGGER trigger_name trigger_action_time trigger_events ON
...
create_view_stmt : CREATE opt_view_algorithm opt_sql_security VIEW view_name
                       opt_column_name_list_p AS select_stmt opt_check_option {
  $$ = new CreateViewStmt();
  $$->case_idx_ = CASE0;
...
| CREATE OR REPLACE opt_view_algorithm opt_sql_security VIEW view_name
        opt_column_name_list_p AS select_stmt opt_check_option {
  $$ = new CreateViewStmt();
  $$->case_idx_ = CASE1;
...
opt_table_option_list : table_option_list {
...
table_option : INSERT_METHOD opt_op_equal NO {
  $$ = new TableOption();
  $$->case_idx_ = CASE0;
  $$->opt_op_equal_ = $2;
}
...
opt_view_algorithm : ALGORITHM OP_EQUAL UNDEFINED {
...
opt_sql_security : SQL SECURITY DEFINER {
  $$ = new OptSqlSecurity();
  $$->case_idx_ = CASE0;
}
...
opt_extra_option : index_algorithm_option {
  $$ = new OptExtraOption();
  $$->case_idx_ = CASE0;
...
index_algorithm_option : ALGORITHM opt_op_equal DEFAULT {
...
lock_option : LOCK opt_op_equal DEFAULT {
  $$ = new LockOption();
  $$->case_idx_ = CASE0;
  $$->opt_op_equal_ = $2;
}
...
drop_index_stmt : DROP INDEX table_name opt_extra_option {
  $$ = new DropIndexStmt();
  $$->case_idx_ = CASE0;
  $$->table_name_ = $3;
  $$->opt_extra_option_ = $4;
...
opt_restrict_or_cascade : RESTRICT {
...
drop_trigger_stmt : DROP TRIGGER opt_if_exist trigger_name {
  $$ = new DropTriggerStmt();
  $$->case_idx_ = CASE0;
  $$->opt_if_exist_ = $3;
  $$->trigger_name_ = $4;
...
drop_view_stmt : DROP VIEW opt_if_exist view_name opt_restrict_or_cascade {
  $$ = new DropViewStmt();
  $$->case_idx_ = CASE0;
  $$->opt_if_exist_ = $3;
  $$->view_name_ = $4;
...
insert_stmt : opt_with_clause INSERT INTO table_name opt_as_alias insert_rest
                  opt_on_conflict {
  $$ = new InsertStmt();
  $$->case_idx_ = CASE0;
  $$->opt_with_clause_ = $1;
...
insert_rest : opt_column_name_list_p select_no_parens {
  $$ = new InsertRest();
  $$->case_idx_ = CASE0;
...
values_list : OP_LP expr_list OP_RP {
  $$ = new ValuesList();
  $$->case_idx_ = CASE0;
  $$->expr_list_ = $2;
};

opt_on_conflict : ON CONFLICT opt_conflict_expr DO NOTHING {
  $$ = new OptOnConflict();
  $$->case_idx_ = CASE0;
...
opt_conflict_expr : OP_LP indexed_column_list OP_RP where_clause {
  $$ = new OptConflictExpr();
  $$->case_idx_ = CASE0;
...
indexed_column : expr opt_order_behavior {
  $$ = new IndexedColumn();
  $$->case_idx_ = CASE0;
  $$->expr_ = $1;
  $$->opt_order_behavior_ = $2;
...
update_stmt : UPDATE IGNORE table_name opt_as_alias SET set_clause_list
                  opt_where_clause opt_order_clause opt_limit_row_count {
  $$ = new UpdateStmt();
  $$->case_idx_ = CASE0;
  $$->table_name_ = $3;
  $$->opt_as_alias_ = $4;
...
| UPDATE table_name opt_as_alias SET set_clause_list opt_where_clause
        opt_order_clause opt_limit_row_count {
  $$ = new UpdateStmt();
  $$->case_idx_ = CASE1;
  $$->table_name_ = $2;
  $$->opt_as_alias_ = $3;
...
alter_action : RENAME TO table_name {
  $$ = new AlterAction();
  $$->case_idx_ = CASE0;
  $$->table_name_ = $3;
  if ($$) {
...
alter_constant_action : DROP PRIMARY KEY {
...
column_def : identifier type_name opt_column_constraint_list {
  $$ = new ColumnDef();
  $$->case_idx_ = CASE0;
  $$->identifier_ = $1;
  $$->type_name_ = $2;
...
opt_reference_clause : opt_foreign_key reference_clause {
...
opt_check : CHECK OP_LP expr OP_RP opt_enforced {
  $$ = new OptCheck();
  $$->case_idx_ = CASE0;
  $$->expr_ = $3;
  $$->opt_enforced_ = $5;
...
reference_clause : REFERENCES table_name opt_column_name_list_p
                       opt_foreign_key_actions opt_constraint_attribute_spec {
  $$ = new ReferenceClause();
  $$->case_idx_ = CASE0;
  $$->table_name_ = $2;
...
opt_foreign_key_actions : foreign_key_actions {
...
foreign_key_actions : MATCH FULL {
...
opt_constraint_attribute_spec : DEFFERRABLE opt_initial_time {
...
opt_initial_time : INITIALLY DEFERRED {
  $$ = new OptInitialTime();
  $$->case_idx_ = CASE0;
}
| INITIALLY IMMEDIATE {
...
opt_column_name_list_p : OP_LP column_name_list OP_RP {
...
set_clause : column_name OP_EQUAL expr {
  $$ = new SetClause();
  $$->case_idx_ = CASE0;
  $$->column_name_ = $1;
  $$->expr_ = $3;
...
cast_expr : CAST OP_LP expr AS type_name OP_RP {
  $$ = new CastExpr();
  $$->case_idx_ = CASE0;
  $$->expr_ = $3;
  $$->type_name_ = $5;
...
unary_expr : OP_SUB operand % prec OP_SUB {
  $$ = new UnaryExpr();
  $$->case_idx_ = CASE0;
  $$->operand_ = $2;
}
...
in_expr : operand opt_not IN OP_LP select_no_parens OP_RP {
  $$ = new InExpr();
  $$->case_idx_ = CASE0;
  $$->operand_ = $1;
  $$->opt_not_ = $2;
...
between_expr : operand BETWEEN operand AND operand % prec BETWEEN {
  $$ = new BetweenExpr();
  $$->case_idx_ = CASE0;
  $$->operand_1_ = $1;
  $$->operand_2_ = $3;
...
exists_expr : opt_not EXISTS OP_LP select_no_parens OP_RP {
  $$ = new ExistsExpr();
  $$->case_idx_ = CASE0;
  $$->opt_not_ = $1;
  $$->select_no_parens_ = $4;
...
function_expr : function_name OP_LP OP_RP opt_filter_clause opt_over_clause {
  $$ = new FunctionExpr();
  $$->case_idx_ = CASE0;
  $$->function_name_ = $1;
...
| function_name OP_LP opt_distinct expr_list OP_RP opt_filter_clause
        opt_over_clause {
  $$ = new FunctionExpr();
  $$->case_idx_ = CASE1;
  $$->function_name_ = $1;
...
opt_filter_clause : FILTER OP_LP WHERE expr OP_RP {
  $$ = new OptFilterClause();
  $$->case_idx_ = CASE0;
  $$->expr_ = $4;
}
...
opt_over_clause : OVER OP_LP window OP_RP {
  $$ = new OptOverClause();
  $$->case_idx_ = CASE0;
  $$->window_ = $3;
}
...
case_clause : WHEN expr THEN expr {
  $$ = new CaseClause();
  $$->case_idx_ = CASE0;
  $$->expr_1_ = $2;
  $$->expr_2_ = $4;
...
comp_expr : operand OP_EQUAL operand {
  $$ = new CompExpr();
  $$->case_idx_ = CASE0;
  $$->operand_1_ = $1;
  $$->operand_2_ = $3;
...
extract_expr : EXTRACT OP_LP datetime_field FROM expr OP_RP {
  $$ = new ExtractExpr();
  $$->case_idx_ = CASE0;
  $$->datetime_field_ = $3;
  $$->expr_ = $5;
...
array_expr : ARRAY OP_LBRACKET expr_list OP_RBRACKET {
  $$ = new ArrayExpr();
  $$->case_idx_ = CASE0;
  $$->expr_list_ = $3;
};

array_index : operand OP_LBRACKET int_literal OP_RBRACKET {
  $$ = new ArrayIndex();
  $$->case_idx_ = CASE0;
  $$->operand_ = $1;
  $$->int_literal_ = $3;
...
opt_if_not_exist : IF NOT EXISTS {
  $$ = new OptIfNotExist();
  $$->case_idx_ = CASE0;
}
| {
...
character_type : character_with_length {
  $$ = new CharacterType();
  $$->case_idx_ = CASE0;
...
character_with_length : character_conflicta OP_LP int_literal OP_RP {
...
character_without_length : character_conflicta {
...
opt_table_constraint_list : table_constraint_list {
...
table_constraint : constraint_name PRIMARY KEY OP_LP indexed_column_list OP_RP {
  $$ = new TableConstraint();
  $$->case_idx_ = CASE0;
  $$->constraint_name_ = $1;
