...
class PostgresClient{
  public:
    ...
    PGconn * connect(){
      string dbname = "dbname=x";
      
      dbname += " connect_timeout=4";
      return PQconnectdb(dbname.c_str());
    }
    ...
    void reset_database(PGconn* conn){
        PQexec(conn, "DROP SCHEMA public CASCADE; CREATE SCHEMA public;");
    }

    void drop_database(PGconn * conn){
      if(counter_ % 2 == 0) PQexec(conn, "DROP DATABASE IF EXISTS test;");
      else PQexec(conn, "DROP DATABASE IF EXISTS test2;");
    }
    
    void create_database(PGconn * conn){
      if(counter_ % 2 == 0) PQexec(conn, "CREATE DATABASE test;");
      else PQexec(conn, "CREATE DATABASE test2;");
    }
    
    bool check_status(PGconn * conn){
      auto res = PQstatus(conn);
      if(res == CONNECTION_OK) return true;
...
BEGIN:
  ...
  auto result = g_psql_client.execute(g_current_input);
